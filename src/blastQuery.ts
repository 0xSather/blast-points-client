...
type PointType = "PHASE2_POINTS" | "PHASE2_GOLD";
...
export class BlastPointsSession {
  ...
  constructor(
    contractAddr: string,
    pointsServerUrl: string = BLAST_MAINNET_POINTS_URL
  ) {
  ...
  async queryPoints(): Promise<PointsBalancesAcross> {
    ...
    const response = await axios.get<PointsResponse>(
      endpoint,
      await this.bearerHeader()
    );
    if (!response.data.success) {
      throw new Error("Error: Point query request failed");
    }
    return {
      PHASE2_POINTS: response.data.balancesByPointType.PHASE2_POINTS,
      PHASE2_GOLD: response.data.balancesByPointType.PHASE2_GOLD,
    };
  }
  ...
  async cancelTransfer(batchId: string) {
    ...
    if (!response.data.success) {
      throw new Error("Error: Transfer cancel request failed");
    }
  ...
  async queryTransfer(batchId: string): Promise<TransferBatchList> {
    const endpoint = `${this.pointsServerUrl}/v1/contracts/${this.contractAddr}/batches/${batchId}`;
    const response = await axios.get<TransferQueryResponse>(
      endpoint,
      await this.bearerHeader()
    );
    if (!response.data.success) {
      throw new Error("Error: Transfer query request failed");
    }
    return response.data.batch;
  }

  private async queryTransfersPage(
    cursor?: string
    ...
    const response = await axios.get<TransferPageResponse>(
      urlWithCursor,
      await this.bearerHeader()
    );
    if (!response.data.success) {
    ...
    return this.transferRequest(
      {
        ...
        transfers,
        secondsToFinalize: this.secondsToFinalize,
      },
      batchId
    );
  }

  async transferDevPoints(
    transfers: PointsTransfer[],
    batchId?: string
  ): Promise<string> {
    return this.transferRequest(
  ...
  private async transferRequest(
    ...
    const response = await axios.put<TransferResponse>(
      endpoint,
      transfers,
      await this.bearerHeader()
    );
    if (!response.data.success) {
      throw new Error("Error: Transfer request failed");
    }
  ...
  private async obtainBearerToken(
...
export const BLAST_TESTNET_POINTS_URL =
  "https://waitlist-api.develop.testblast.io";
...
async function signMessageERC191(
  message: string,
  operatorKey: string
): Promise<string> {
